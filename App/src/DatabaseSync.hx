...
class DatabaseSync 
{
	...
	static public function DBSync (sqlite_type:String)
	{
		...
		if (sqlite_type == "run") {
			...
			for (entries in 0...run_entry_list.length) {
				...
				var entry_item_str = entry_item.toString();
				var entry_item_str_mod = StringTools.replace(entry_item_str, "app_data_run(subject_id:" + _id + ",timestamp:", "");
				var entry_item_str_mod2 = StringTools.replace(entry_item_str_mod, ")", "");
				...
				var app_run = AppDataRun.manager.get({subject_id: _id, timestamp: entry_time});
	...
	static public function SyncUserData (identifier:Int)
	{
		...
		var subj_data = AppDataTrial.manager.select($subject_id == identifier);
